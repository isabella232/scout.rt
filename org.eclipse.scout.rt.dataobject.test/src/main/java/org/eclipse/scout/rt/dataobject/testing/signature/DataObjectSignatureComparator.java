...
public class DataObjectSignatureComparator {
  ...
  protected static final Pattern VALUE_TYPE_PATTERN = Pattern.compile(""
      + "("
      + StringUtility.join("|",
          DataObjectSignatureGenerator.VALUE_TYPE_DO_PREFIX,
          DataObjectSignatureGenerator.VALUE_TYPE_DO_INTERFACE_PREFIX,
          DataObjectSignatureGenerator.VALUE_TYPE_ENUM_PREFIX,
          DataObjectSignatureGenerator.VALUE_TYPE_ID_PREFIX,
          DataObjectSignatureGenerator.VALUE_TYPE_CLASS_PREFIX)
      + ")"
      + "\\["
      + "([\\w.\\-]+)"
      + "]");
  ...
  /**
   * Normalizes {@link AttributeDataObjectSignatureDo#getValueType()} because referenced other type names, enum names...
   * that might need to be translated to the new type names if renamed.
   */
  protected String normalizeValueType(String valueType) {
    Matcher matcher = VALUE_TYPE_PATTERN.matcher(valueType);
